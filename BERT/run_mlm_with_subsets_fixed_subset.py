...
def main():
    ...
    if not args.preprocessed:
        ...
        dataset2=load_from_disk("wikitext-103-processed-first-sentences")
    ...
    full_dataloader=DataLoader(
        dataset2["train"], shuffle=False, collate_fn=data_collator2, batch_size=args.per_device_eval_batch_size
    )
    ...
    model, model2, optimizer, warmstart_dataloader, full_dataloader, subset_dataloader, eval_dataloader = accelerator.prepare(
        model, model2, optimizer, warmstart_dataloader, full_dataloader, subset_dataloader, eval_dataloader)
    ...
    if (args.num_warmstart_epochs>=0) or (args.resume_from_checkpoint):
        ...
        if args.selection_strategy == 'Random-Online':
            ...
            if accelerator.is_main_process:
                partition_indices, greedyIdx, gains = subset_strategy.select(num_samples, batch_indices, representations, parallel_processes=args.parallel_processes, return_gains=True)
                init_subset_indices=[[]]
                init_subset_indices=[greedyIdx]
                # i=0
                # for p in gains:
